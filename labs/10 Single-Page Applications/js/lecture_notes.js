...
function loadList() {
	...
	for (i = 0; i < notes.length; i += 1) {
		...
		row.id = i;
		row.innerHTML = '<td><a onclick="display(this)" href="#">'
				+ notes[i].title
				+ '</a></td><td><a onclick="rem(this)" class="delete" href="#">delete</a></td>';
...
function display(element) {
	...
	document.querySelector('#editPage input').onkeyup = function() {
		notes[id].title = document.querySelector('#editPage input').value;
		loadList();
	};

	document.querySelector('#editPage textarea').onkeyup = function() {
		notes[id].note = document.querySelector('#editPage textarea').value;
	};
}
